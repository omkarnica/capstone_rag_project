"""
End-to-end filings build pipeline.

Calling module:

    src.filings.__init__ lazily exposes run_filings_pipeline().
...
Pipeline flow:
    ...
    run_filings_pipeline()
      |
      v
    ingestion.ingestion_filing()
      |
      v
    chunking.docling_json_to_pinecone_chunks()
      |
      v
    chunking.save_chunks_to_json()
      |
      v
    raptor.run_raptor_pipeline()
      |
      v
    raptor_verification.verify_raptor_tree()

Retrieval is not part of this build flow. Query-time retrieval starts from
src.filings.raptor_retrieval after the RAPTOR vectors are available.
"""
